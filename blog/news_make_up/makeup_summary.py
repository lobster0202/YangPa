...
def main():
    ...
    # 1. 객체 생성
    blogLink = blog_link.BlogLink()
    linkCrawling = link_crawling.LinkCrawling()
    newsContent = news_content.NewsContent()

    # 2. 링크 추출
    blog_links = blogLink.get_post_links()
    news_links = linkCrawling.news_link(blog_links)
    ...
    for news_link in news_links:
        title, content = newsContent.news_crawling(news_link)
